...
export class PurchaseInvoice extends Invoice {
  ...
  async getPosting() {
    ...
    if (this.isReturn) {
      await posting.debit(this.account!, this.baseGrandTotal!.abs());
    } else {
    ...
    for (const item of this.items!) {
      if (this.isReturn) {
        await posting.credit(item.account!, item.amount!.mul(exchangeRate).abs());
        continue;
      }
    ...
    if (this.taxes) {
      for (const tax of this.taxes) {
        if (this.isReturn) {
          await posting.credit(tax.account!, tax.amount!.mul(exchangeRate).abs());
          continue;
        }
